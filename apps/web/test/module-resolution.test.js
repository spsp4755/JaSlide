...
test('a table can be inserted on a slide, at a chosen size', () => {
    const editor = fs.readFileSync(path.join(SRC, 'app', 'editor', '[id]', 'page.tsx'), 'utf8');

    // Size chosen before inserting: a fixed 3x3 cannot grow afterwards.
    assert.match(editor, /const insertSceneTable = \(rows: number, columns: number\)/);
...
test('manual editing has the keys Google Slides users expect', () => {
    const editor = fs.readFileSync(path.join(SRC, 'app', 'editor', '[id]', 'page.tsx'), 'utf8');

    // Delete/Ctrl+D/nudging act on whatever the scene canvas has selected, for
    // either deck kind alike — scene-canvas.test.js covers the drag/resize/snap
    // geometry itself.
    assert.match(editor, /if \(selectedNativeObjectId\) deleteSelectedObject\(\)/);
    assert.match(editor, /if \(selectedNativeObjectId\) duplicateSelectedObject\(\)/);
    assert.match(editor, /nudgeBox\(/);
    assert.match(editor, /onSceneCommand\(\{ objectId: selectedObject\.id, patch: \{ x: box\.left, y: box\.top \} \}\)/);
});
